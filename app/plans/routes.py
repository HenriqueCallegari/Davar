...
@bp.route("/planos")
def listar_planos():
    planos = current_app.services.plans.list_plans()
...
def mostrar_plano(plano_id: int):
    ...
    dados = current_app.services.plans.get_plan_day(plano_id, dia)
    if dados is None:
...
def mostrar_estatisticas(plano_id: int):
    dados = current_app.services.plans.get_statistics_page(plano_id)
    if dados is None:
        return render_template("pages/error.html", code=404,
                               mensagem="Plano não encontrado."), 404
...
def salvar_progresso(plano_id: int):
    ...
    resultado = current_app.services.plans.update_progress(plano_id, livro, capitulo, concluido)
...
def salvar_nota(plano_id: int, dia: int):
    ...
    resultado = current_app.services.plans.save_note(plano_id, dia, texto)
    if resultado is None:
